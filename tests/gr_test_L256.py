"""
Full-scale G(r) test: L=256, 1000 production chunks.

Uses analytical mu_coex — no Stage 1 / manage.csv needed.
Run via: bash scripts/submit_gr_test_L256.sh [output_dir]

Writes to --output-dir (default: ~/gr_test_L256/):
    snapshots/replica_00000/   -- 1000 .npy chunk snapshots
...
# ── Physics parameters ────────────────────────────────────────────────────────
EPSILON   = -1.5    # binding energy (negative = attractive)
DELTA_MU  = 0.0     # equilibrium
K         = 1.0
SCHEME    = "homo"
BETA      = 1.0
...
L         = 256
EQ_TIME   = 50_000   # equilibration sweeps (larger system needs more)
PROD_TIME = 100_000  # total production sweeps
N_CHUNKS  = 1000     # snapshots saved (100 sweeps each)
SEED      = 42
...
def run_simulation(outdir: str) -> list[str]:
    ...
    print(f"[gr_test_L256] mu_coex (analytical) = {mu:.6f}", flush=True)
    print(f"[gr_test_L256] L={L}  eq_time={EQ_TIME}  prod_time={PROD_TIME}  "
          f"n_chunks={N_CHUNKS}  chunk_time={PROD_TIME / N_CHUNKS:.1f}", flush=True)
    ...
    print(f"[gr_test_L256] equilibrating …", flush=True)
    simulate(state, boundary, chain, [], [Time(EQ_TIME)], SEED, scratch)
    state = load.final_state(scratch)
    print("[gr_test_L256] equilibration done", flush=True)
    ...
    for i in range(N_CHUNKS):
        ...
        rows.append({
            "run_id": 0, "replica_id": 0, "chunk": i,
            "snapshot_path": os.path.relpath(path, outdir),
            "time": t, "chunk_time": chunk_time,
            "epsilon": EPSILON, "delta_f": DELTA_F, "delta_mu": DELTA_MU,
            "k": K, "scheme": SCHEME,
            "Lx": L, "Ly": L,
            "mu": mu, "mu_coex_FITTED": mu,
            "beta": BETA, "eq_time": EQ_TIME,
            "prod_time": PROD_TIME, "prod_chunks": N_CHUNKS, "seed": SEED,
        })

        if (i + 1) % 100 == 0:
            print(f"[gr_test_L256] {i + 1}/{N_CHUNKS} chunks done", flush=True)
    ...
    print(f"[gr_test_L256] metadata written → {meta_path}", flush=True)

    return snap_paths


def plot_G_r(r_centers: np.ndarray, G_r: np.ndarray, fit: dict, output_path: str) -> None:
    ...
    ax = axes[0]
    ax.plot(r_centers[1:], G_r[1:], "o", ms=3, label="G(r) data")
    ...
    ax = axes[1]
    pos = G_r[1:] > 0
    ax.semilogy(r_centers[1:][pos], G_r[1:][pos], "o", ms=3, label="G(r) data")
    ...
    print(f"[gr_test_L256] plot saved → {output_path}", flush=True)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output-dir", default=os.path.expanduser("~/gr_test_L256"))
    args = parser.parse_args()

    outdir = os.path.abspath(args.output_dir)
    os.makedirs(outdir, exist_ok=True)
    print(f"[gr_test_L256] output dir: {outdir}", flush=True)

    snap_paths = run_simulation(outdir)

    print(f"[gr_test_L256] computing G(r) from {len(snap_paths)} snapshots …", flush=True)
    ...
    print(
        f"[gr_test_L256] xi = {fit['xi']:.4f} ± {fit['xi_err']:.4f}  "
        f"A = {fit['A']:.4f}  fit_points = {fit['n_points']}",
        flush=True,
    )
    if fit.get("warning"):
        print(f"[gr_test_L256] WARNING: {fit['warning']}", flush=True)
    ...
    print("[gr_test_L256] done", flush=True)
